...
3. 根据实际安装选择反射球开口在 TCP 中的方向，设置最大入射角和整周检查步长。
4. 点击“预览筛选”，绿色点表示保留，红色点表示删除。三维视图中的橙色球为
   跟踪仪原点，蓝色线框球为覆盖当前工作模型的测量范围，红色线框球为
   0.8 m 近场排除区。
5. 勾选“生成时启用整周可见性筛选”后再生成文件。程序固定其他关节，将
   J4（C轴）均匀模拟旋转 360°；每个检查位置都满足 0.8～160 m 测距和
   反射球入射角限制时才保留该关节点。

可见性计算使用五轴正解返回的 `[x,y,z,rx,ry]`，其中 `rx/ry` 按运动学 DLL
的实际输出使用弧度。当前约定反射球中心与 TCP 重合，姿态按固定轴 X 后 Y
将所选开口方向转换到机器人基坐标；如现场反射球中心相对 TCP 还有固定偏移，
应先在运动学工具定义中补偿该偏移。
