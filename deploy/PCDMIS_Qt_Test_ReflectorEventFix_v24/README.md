...
连接到 PC-DMIS 后，程序通过 ActiveQt 通用 COM 事件通道监听
`Machine.ErrorMsg`。当 PC-DMIS 报告
...
中使用 `MachineToPartMatrix` 完成转换，再把结果发送给主线程。错误信息订阅和
坐标读取使用同一个工作线程中的 `Machine` 对象，线程迁移不会改变
`ErrorMsg(ErrorType=2)` 的监听逻辑。
