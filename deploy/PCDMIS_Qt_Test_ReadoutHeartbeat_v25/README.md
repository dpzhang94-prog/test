...
连接到 PC-DMIS 后，程序监视 `Machine.ReadoutExt` 实时读数心跳，并监听
`Machine.ErrorMsg` 作为补充。PC-DMIS 2017 R1 帮助将 `ReadoutExt` 定义为
“控制器收到读数时触发，并与 XYZ 显示匹配”；正常收到首个心跳后，如果心跳中断
超过 1 秒，程序会判定反射球失锁，把 XYZ 显示为红色、停止发布可能已过期的
`ProbePosition`，并在日志中提示当前坐标无效。重新收到心跳后会自动恢复。
如果 PC-DMIS 主动报告 `ERROR_TYPE_REFLECTOR_NOT_FOUND`（`ErrorType=2`），
也会进入相同的无效状态。诊断不弹出警告框，也不中止后台轮询。

## 线程结构

`PcdmisClient` 是主线程中的界面门面，只保存连接、轮询和仪器原点等少量状态；
`PcdmisWorker` 位于专用工作线程，拥有全部 `QAxObject`、轮询定时器和
PC-DMIS COM 调用。界面操作通过队列信号向 Worker 发出请求，坐标、错误和日志
也通过队列信号返回主线程显示，因此拖动窗口时不会再由同步 COM 读取阻塞界面。

仪器坐标模式由 Worker 直接返回 `ProbePosition`；对齐坐标模式则先在 Worker
中使用 `MachineToPartMatrix` 完成转换，再把结果发送给主线程。`ReadoutExt`
心跳、错误信息订阅和坐标读取使用同一个工作线程中的 `Machine` 对象，确保 COM
事件和轮询状态按顺序处理。
