...
生成点数表示进入过滤前的候选点数量。程序将 J1～J5 的有效范围分别划分为
指定数量的区间，在各区间中点取值并独立打乱组合，使用拉丁超立方方式生成
分散的关节姿态。固定关节保持最小值，J6 固定为 0°。整数或 0.5 精度造成
重复时，程序会重新打乱并补足唯一点；无法补足时会提示调整范围或精度。
STL 可达域过滤、J6 整周可见性过滤及 TXT、CSV、JBR 输出方式保持不变。
...
5. “预览筛选”和最终筛选明细位于底部“生成设置”区域。预览按当前复选框
   状态依次执行拉丁超立方生成、去重、扩散区域过滤和 J6 整周过滤，并显示每个
