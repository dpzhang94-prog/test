...
2. 打开测试点页面，点击“获取/刷新位置”。程序复用主界面的 Automation
   ...
   如果只想离线检查界面和筛选流程，可点击“使用测试位置”：加载 STL 后
   程序会按模型范围自动放置模拟跟踪仪；未加载 STL 时使用
   `X=3000, Y=0, Z=1500 mm`。模拟位置不会访问 PC-DMIS，不能作为实测数据。
3. 输入 J6=0° 时靶球中心相对 TCP 的 X/Y/Z 安装偏移，并根据实际安装选择
   靶球开口方向。一般径向安装可选“沿 TCP → 靶球（径向）”。
4. 设置最大入射角和 J6 整周检查步长。
5. 点击“预览筛选”，绿色点表示保留，红色点表示删除。三维视图中的橙色球为
...
按固定轴 X 后 Y 转换到机器人基坐标系，再叠加绕 TCP 局部 +Z 轴的 J6 旋转。
