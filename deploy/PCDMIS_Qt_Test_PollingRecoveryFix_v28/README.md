...
矩阵坐标变换本身不依赖探头读数窗口设置，也不需要把 `PointData` 对象传回
PC-DMIS，适用于 PC-DMIS 2017 R1/AT403 环境。

连续读取时，程序在 PC-DMIS 工作线程中比较坐标转换前的原始
`Machine.ProbePosition`。XYZ 连续 10 次完全相同后，状态切换为
`Lost`，主界面打印“反射球丢失。”并将 XYZ 显示为红色。即使读取
`ProbePosition` 连续失败，后台轮询也不会停止；连续失败 3 次时同样切换为
`Lost`，并继续重试。后续读取成功且原始坐标重新发生变化时，状态切换回
`Tracking`，主界面打印“反射球重新捕获，跟踪恢复。”并恢复默认颜色。
状态只在发生变化时通过信号发送，因此不会重复刷日志。

`Machine.ReadoutExt` 和 `Machine.ErrorMsg(ErrorType=2)` 仍保留为补充诊断，
但它们不再控制主界面的坐标颜色或采集按钮。跟踪状态信号在主线程中只负责
打印消息和切换 XYZ 颜色，不改变原有坐标显示、圆周点采集及计算流程。

## 线程结构

`PcdmisClient` 是主线程中的界面门面，只保存连接、轮询和仪器原点等少量状态；
`PcdmisWorker` 位于专用工作线程，拥有全部 `QAxObject`、轮询定时器和
PC-DMIS COM 调用。界面操作通过队列信号向 Worker 发出请求，坐标、跟踪状态、
错误和日志也通过队列信号返回主线程显示，因此拖动窗口时不会再由同步 COM
读取阻塞界面。

仪器坐标模式由 Worker 直接返回 `ProbePosition`；对齐坐标模式则先在 Worker
中使用 `MachineToPartMatrix` 完成转换，再把结果发送给主线程。原始坐标冻结
判断、`ReadoutExt` 心跳、错误信息订阅和坐标读取使用同一个工作线程中的
`Machine` 对象，确保 COM 事件和轮询状态按顺序处理。
